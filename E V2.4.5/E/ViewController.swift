...
class ViewController: UIViewController {
    ...
    @IBAction func RESET(_ sender: Any) {
        ...
        //shake smol nutella
        self.view.addSubview(NUTELLAA)
        let BEEGNUTELLASHAKEANIMATION = CABasicAnimation(keyPath: "position")
        BEEGNUTELLASHAKEANIMATION.duration = 0.07
        BEEGNUTELLASHAKEANIMATION.repeatCount = 1
        BEEGNUTELLASHAKEANIMATION.autoreverses = true
        BEEGNUTELLASHAKEANIMATION.fromValue = NSValue(cgPoint: CGPoint(x: NUTELLAA.center.x + CGFloat(xrandom), y: NUTELLAA.center.y + CGFloat(yrandom)))
        BEEGNUTELLASHAKEANIMATION.toValue = NSValue(cgPoint: CGPoint(x: NUTELLAA.center.x - CGFloat(xrandom), y: NUTELLAA.center.y + CGFloat(yrandom)))
        NUTELLAA.layer.add(BEEGNUTELLASHAKEANIMATION, forKey: "position")
        self.view.sendSubviewToBack(NUTELLAA)
